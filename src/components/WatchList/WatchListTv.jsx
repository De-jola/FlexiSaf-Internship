...
class WatchListTv extends React.Component {
  constructor() {
    super();
    this.state = {
      movieList: [],
  ...
  componentDidMount() {
    const url =
      "https://api.themoviedb.org/3/account/21561650/watchlist/tv?language=en-US&page=1&sort_by=created_at.asc";
    this.getMovie(url);
  }
  ...
  render() {
    ...
    return (
      <>
        <h3 className="text-2xl font-bold text-white">Tv </h3>
        {movieList.length === 0 ? (
          <p className="text-white">Add Tv shows to your list</p>
        ) : (
